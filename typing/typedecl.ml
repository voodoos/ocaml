      ...
      let targs, tret_type, args, ret_type, univars, discourse =
        Ctype.with_local_level_generalize_if closed begin fun () ->
          ...
          let tret_type, tret_discourse =
            transl_simple_type_with_discourse env ?univars ~closed sret_type in
          ...
          let discourse =
            Discourse_types.union args_discourse tret_discourse
          in
...
let transl_declaration env sdecl (id, uid) =
  ...
  let discourse, constraints = List.fold_left_map
    (fun acc_d (sty, sty', loc) ->
      let ct, d = transl_simple_type_with_discourse env ~closed:false sty in
      let ct', d' = transl_simple_type_with_discourse env ~closed:false sty' in
  ...
  in
  let (tkind, kind, type_discourse) =
    match sdecl.ptype_kind with
      | Ptype_abstract ->
        Ttype_abstract, Type_abstract Definition, discourse
      | Ptype_external name ->
      ...
      | Ptype_variant scstrs ->
        ...
        let make_cstr acc_discourse scstr =
          ...
          in
          let discourse =
            Discourse_types.union acc_discourse cd_discourse
          in
  ...
  begin
    let (tman, man, type_discourse) = match sdecl.ptype_manifest with
      ...
      | Some sty ->
        ...
        Discourse_types.union type_discourse d
    in
...
let check_coherence env loc dpath decl =
  match decl with
    { type_kind = (Type_variant _ | Type_record _| Type_open);
      ...
      | _ ->
          Error.log_and_raise loc (Definition_mismatch (ty, env, None))
      end
  | _ -> ()
...
let transl_type_decl env rec_flag sdecl_list =
      ...
      let decl = tdecl.typ_type in
       match Ctype.closed_type_decl decl with
         Some var ->
           ...
           if not (Typing_recovery.erroneous_type_check var) then
  ...
  let decls =
    ...
    | Typedecl_immediacy.Error (loc, err) ->
        Error.log_and_raise loc (Immediacy err)
...
let error_if_has_deep_native_repr_attributes core_type =
  ...
  let this_iterator =
    { default_iterator with typ = fun iterator core_type ->
      begin
        match
          get_native_repr_attribute core_type.ptyp_attributes
            ~global_repr:None
        with
...
let rec parse_native_repr_attributes env core_type ty ~global_repr =
  ...
  | Ptyp_functor _, Tfunctor _, _ ->
      Error.log_and_raise core_type.ptyp_loc (Type_cannot_be_external ty)
...
let transl_prim_desc env loc primdesc =
  ...
  | Pprim_decl (pprim_type, pprim_prim) ->
    let cty, val_discourse = Typetexp.transl_type_scheme env pprim_type in
    let ty = cty.ctyp_type in
    let v =
      let global_repr =
        match
    ...
    let desc =
      {
       prim_id = id;
       prim_name = primdesc.pprim_name;
       prim_kind = Tprim_decl (cty, pprim_prim); prim_val = v;
       prim_loc = primdesc.pprim_loc;
       prim_attributes = primdesc.pprim_attributes;
      }
    in
    desc, newenv
    ...
    (match v.val_kind with
     | Val_prim _ ->
       let cty, v =
         ...
         | None -> None, v
         | Some pprim_type ->
           let cty, _discourse = Typetexp.transl_type_scheme env pprim_type in
           ...
           | exception (Ctype.Matches_failure (env, err)) ->
               Error.log_and_raise cty.ctyp_loc
                 (Primitive_type_mismatch (env, err))
       in
       let (id, newenv) =
         Env.enter_value primdesc.pprim_name.txt v env
           ~check:(fun s -> Warnings.Unused_value_declaration s)
       ...
       let desc =
         {
          prim_id = id;
          prim_name = primdesc.pprim_name;
          prim_kind = Tprim_alias (cty, path, pprim_ident);
          prim_val = v;
          prim_loc = primdesc.pprim_loc;
          prim_attributes = primdesc.pprim_attributes;
         }
       in
       desc, newenv
     | Val_reg | Val_ivar _ | Val_self _ | Val_anc _ as val_kind ->
         Error.log_and_raise pprim_ident.loc
           (Primitive_alias_does_not_refer_to_primitive val_kind))
...
let transl_with_constraint id ?fixed_row_path ~sig_env ~sig_decl ~outer_env
  ...
  Option.iter (fun p -> set_private_row env sdecl.ptype_loc p new_sig_decl)
    ...
    begin match Ctype.closed_type_decl new_sig_decl with
      ...
      | Some var ->
          Error.log_and_raise loc
            (Unbound_type_var {var; params; decl = new_sig_decl})
  end;
...
let report_error ~loc = function
  ...
  | Primitive_alias_does_not_refer_to_primitive val_kind ->
      Location.errorf ~loc
        ...
        (match val_kind with
         | Val_prim _ ->
           Misc.fatal_error "Typedecl.report_error: value is a primitive"
         | Val_reg -> "a regular value"
         | Val_ivar _ -> "an instance variable"
         | Val_self _ -> "the self object"
         | Val_anc _ -> "an ancestor object")
  | Primitive_type_mismatch (env, err) ->
      let msg = Format_doc.Doc.msg in
      Location.errorf ~loc
        "@[<v>The type of this alias does not match that of the aliased \
         primitive.@,%t@]" @@ fun ppf ->
        Errortrace_report.unification ppf env err
          (msg "Type")
