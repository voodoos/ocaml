...
module Shape = struct
  ...
  (* X.Y.Z = Pdot (Pdot (Ident Z, "Y"), "X") -> Proj(Proj(Proj(ShapeX, ("X", mod)), ("Y", mod)), ("Z", mod)) *)
  let module_proj_of_path ~root path =
    let rec aux kind path =
      let kind = Option.value ~default:Sig_component_kind.Module kind in
      match path with
      | Path.Pdot (path, name) ->
        let t = aux None path in
        Proj (t, (name, kind))
      | Path.Pident id ->
        let name = Ident.name id in
        Proj(root, (name, kind))
      | Path.Papply _ -> failwith "TODO @ulysse `Papply` not implemented"
    in
    aux (Some Sig_component_kind.Module) path

  let make_empty_sig () = Abs(fresh_var (), Struct Item.Map.empty)

  let make_external s = Comp_unit s
