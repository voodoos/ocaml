...
module Merge = struct
  ...
  *)
  ...
  (** Called after an item has been patched (i.e. rewrote or removed).

      - [~replace] is an optional substitution, used for destructive constraints
      to replace all occurrences of the destructed item by its new
      definition. Non-destructive constraint leave this argument to [None]
  ...
  let post_process ~approx ~replace ?(invalid_alias=None) loc lid env paths sg =
    let sg =
      ...
      | Some replace ->
         (* Check that the substitution would not make the signature
            ill-formed *)
         if not approx then
           check_usage_after_substitution ~loc ~lid env paths ~invalid_alias sg;
...
and transl_modtype_aux env smty =
  ...
  match smty.pmty_desc with
    Pmty_ident lid ->
      ...
      Discourse_types.singleton (Shape.Sig_component_kind.Module_type, path)
  | Pmty_alias lid ->
      let path = transl_module_alias loc env lid.txt in
      Discourse.use_module env lid path;
      mkmty (Tmty_alias (path, lid)) (Mty_alias path) env loc
        smty.pmty_attributes,
      Discourse_types.singleton (Shape.Sig_component_kind.Module, path)
  | Pmty_signature ssg ->
  ...
  | Pmty_typeof smod ->
      ...
      let discourse =
        ...
        | _ -> empty_discourse
      in
      mkmty (Tmty_typeof tmty) mty env loc smty.pmty_attributes, discourse
  | Pmty_extension ext ->
      raise (Error_forward (Builtin_attributes.error_of_extension ext))
  ...
  let constr, (path, lid, sg), discourse = match constr with
    ...
    | Pwith_modsubst (l,l') ->
        ...
        (constr,
         ...
         Discourse_types.add
           (Shape.Sig_component_kind.Module, path) discourse)
  ...
  let rec transl_sig env sg =
    ...
    | item :: srem -> begin
        ...
        | (item, sg, newenv) ->
            ...
            item :: trem, sg @ rem, finalenv
      end
  and transl_sig_item env item =
    ...
    | Psig_primitive sdesc ->
        ...
        mksig (Tsig_primitive tdesc) env loc,
    ...
    | Psig_typesubst sdecls ->
        ...
        mksig (Tsig_typesubst decls) env loc,
        [],
        newenv
    | Psig_typext styext ->
    ...
    | Psig_exception sext ->
        ...
        Signature_names.check_typext names constructor.ext_loc
          constructor.ext_id;
        mksig (Tsig_exception ext) env loc,
    ...
    | Psig_module pmd ->
        ...
        mksig (Tsig_module {md_id=id; md_name=pmd.pmd_name;
                            ...
                            md_attributes=pmd.pmd_attributes}) env loc,
        (match id with
         | None -> []
    ...
    | Psig_modsubst pms ->
        ...
        let md =
          if not aliasable then
            ...
            let md_discourse =
              Discourse_types.singleton
                (Shape.Sig_component_kind.Module, path)
            in
        ...
        mksig (Tsig_modsubst {ms_id=id; ms_name=pms.pms_name;
                              ...
                              ms_attributes=pms.pms_attributes}) env loc,
        [],
        newenv
    | Psig_recmodule sdecls ->
          let (tdecls, newenv) =
          ...
          map_rec (fun rs (id, md, uid) ->
              ...
              Sig_module(id, Mp_present, d, rs, Exported)) decls [], newenv
    | Psig_modtype pmtd ->
    ...
    | Psig_modtypesubst pmtd ->
       ...
       mksig (Tsig_modtypesubst mtd) env loc, [], newenv
    | Psig_open sod ->
    ...
    | Psig_attribute x ->
        ...
        mksig (Tsig_attribute x) env loc, [], env
...
let package_subtype env pack1 pack2 =
  let mkmty pack =
    let fl =
      List.filter (fun (_,t) ->
          Ctype.closed_type_expr t) pack.pack_constraints in
  ...
  let coercion, shape =
    try
      Includemod.modtypes_constraint ~shape ~loc:arg.mod_loc env ~mark
        arg.mod_type mty
    with Includemod.Error msg ->
      Error.log_and_raise arg.mod_loc env (Not_included msg)
  in
  { mod_desc = Tmod_constraint(arg, mty, explicit, coercion);
    mod_type = mty;
    mod_env = env;
...
and type_module_aux ~alias ~strengthen ~funct_body anchor env smod =
  ...
  match smod.pmod_desc with
    Pmod_ident lid ->
      ...
      in
      let discourse_item = (Shape.Sig_component_kind.Module, path) in
      let discourse_alias = Some (lid, discourse_item) in
      md, shape, Discourse_types.singleton discourse_item, discourse_alias
  | Pmod_structure sstr ->
  ...
  let rec type_struct env shape_map sstr =
    ...
    | item :: srem -> begin
        ...
        | (str, sg, shape_map, newenv) ->
            ...
            let (trem, rem, shape_map, finalenv) =
              type_struct newenv shape_map srem
            in str :: trem, sg @ rem, shape_map, finalenv
      end
  in
  let delayed () =
...
and type_str_item ~names ~toplevel ~funct_body anchor env shape_map
  ...
  let desc, sg, shape_map, new_env =
    ...
    | Pstr_value(rec_flag, sdefs) ->
        ...
        newenv
    | Pstr_val sdesc ->
        Error.log_and_raise sdesc.pval_loc env Val_in_structure
    ...
    | Pstr_include sincl ->
        ...
        in
        let sg =
          let ghostify (loc : Location.t) = { loc with loc_ghost = true } in
          let update_alias =
            match discourse_alias with
            | None -> fun id ->
                let name = Ident.name id in
                let lid = { Location.txt = Longident.Lident name;
                            loc = ghostify modl.mod_loc } in
                let path = Pident id in
                Some (lid, (Shape.Sig_component_kind.Module, path))
            | Some (lid, (m, path)) -> fun id ->
                let name = Ident.name id in
                let lid = { Location.txt =
                              Longident.Ldot (lid, Location.mknoloc name);
                            loc = ghostify lid.Location.loc } in
