  ...
  let rec transl_sig env sg =
    ...
    | item :: srem ->
        ...
        | Psig_value sdesc ->
            ...
            Env.register_uid tdesc.val_val.val_uid tdesc.val_name.loc;
        ...
        | Psig_type (rec_flag, sdecls) ->
            ...
            List.iter (fun td ->
              ...
              if not (Btype.is_row_name (Ident.name td.typ_id)) then
                Env.register_uid td.typ_type.type_uid td.typ_name.loc
            ) decls;
            let (trem, rem, final_env) = transl_sig newenv srem in
            let sg =
        ...
        | Psig_typext styext ->
            ...
            List.iter (fun ext ->
              ...
              Env.register_uid ext.ext_type.ext_uid ext.ext_name.loc
            ) constructors;
            let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        | Psig_module pmd ->
            ...
            let id, newenv =
              ...
              | Some name ->
                ...
                Some id, newenv
            in
            Env.register_uid md.md_uid pmd.pmd_name.loc;
            let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        | Psig_modsubst pms ->
            ...
            Env.register_uid md.md_uid pms.pms_name.loc;
            let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        | Psig_recmodule sdecls ->
            ...
            List.iter (fun (id, md, uid) ->
              ...
              Env.register_uid uid md.md_name.loc
            ) decls;
            let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        | Psig_modtype pmtd ->
            ...
            Env.register_uid decl.mtd_uid mtd.mtd_name.loc;
            let (trem, rem, final_env) = transl_sig newenv srem in
        ...
        | Psig_class cl ->
            ...
            List.iter (fun cls ->
              ...
              Env.register_uid cls.cls_decl.cty_uid cls.cls_id_loc.loc;
            ) classes;
            let (trem, rem, final_env) = transl_sig newenv srem in
            let sg =
              map_rec (fun rs cls ->
...
and type_structure ?(toplevel = false) funct_body anchor env sstr =
  ...
  let type_str_item env shape_map {pstr_loc = loc; pstr_desc = desc} =
        ...
        let items, shape_map =
          List.fold_left
            (fun (acc, shape_map) (id, { Asttypes.loc; _ }, _typ)->
              ...
              Env.register_uid vd.val_uid loc;
    ...
    | Pstr_primitive sdesc ->
        ...
        Env.register_uid desc.val_val.val_uid desc.val_name.loc;
        Tstr_primitive desc,
    ...
    | Pstr_module {pmb_name = name; pmb_expr = smodl; pmb_attributes = attrs;
        ...
        Env.register_uid md_uid name.loc;
    ...
    | Pstr_recmodule sbind ->
        ...
        let shape_map =
          List.fold_left (fun map (id, mb, uid, shape) ->
            Env.register_uid uid mb.mb_name.loc;
