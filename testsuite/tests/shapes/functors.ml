...
module Falias (X : S) = X
[%%expect{|
{("Falias", module) -> .4: Abs(X/94, .3: X/94);
 }
...
{("Finclude", module) ->
     .6:
     Abs(X/98,
         {("t", type) -> .5: X/98 . "t"[type];
          ("x", value) -> .5: X/98 . "x"[value];
          });
 }
...
{("Big_to_small1", module) ->
     .40:
     Abs(shape-var/200,
         <internal>: {("t", type) -> <internal>: shape-var/200 . "t"[type];
                      });
 }
...
{("Big_to_small2", module) ->
     .42:
     Abs(shape-var/206,
         {("t", type) -> .41: <internal>: shape-var/206 . "t"[type];
          });
 }
